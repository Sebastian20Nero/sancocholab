...
async function main() {
    ...
    for (const [key, group] of Object.entries(nitGroups)) {
        if (group.length > 1) {
            console.log(`NIT duplicado: ${key} (${group.length} veces) - IDs: ${group.map(g => g.idProveedor).join(', ')}`);
            dupProvCount++;
        }
    }
    for (const [key, group] of Object.entries(nameGroups)) {
        if (group.length > 1) {
            console.log(`Nombre duplicado: ${key} (${group.length} veces) - IDs: ${group.map(g => g.idProveedor).join(', ')}`);
            dupProvCount++;
        }
    }
    ...
    for (const [key, group] of Object.entries(prodNameGroups)) {
        if (group.length > 1) {
            console.log(`Producto duplicado: ${key} (${group.length} veces) - IDs: ${group.map(g => g.idProducto).join(', ')}`);
            dupProdCount++;
        }
    }
